...
def get_document_sentences(api_request_text):
    ...
    for doc in json.loads(res.read().decode('utf-8')):
        sentences[doc["identifier"].split("/")[1][7:].split(".")[0]] = doc["matched_sentences"]
    return sentences
...
def main():
    ...
    print(*[paper_id for paper_id in sentences.keys()])
    for sent_row in sent_rows:
        sent_row = [re.sub('\\t', ' ', re.sub('\\s+', ' ', sent.replace('\n', '')).strip()) if sent else "" for sent in
                    sent_row]
        print(*sent_row, sep="\t")
